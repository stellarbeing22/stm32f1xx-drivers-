...
int main(void)
{
	...
	while(1)
	{
		...
		//Sending the size of the data in the first byte (as uint8_t is 1 byte long,
		//keep the length of user_data string <255 characters, for larger strings,
		//change the size to uint16_t or uint32_t.

		for (uint8_t i = 0; i < datasize; i++)
		{
		    SPI_SendData(SPI1, (uint8_t*)&user_data[i], 1);
		    SPI_RecieveData(SPI1, (uint8_t*)&Rxbuffer[i], 1);
		}
